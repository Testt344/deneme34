...
RegisterNetEvent("complete_carplay:displayText")
AddEventHandler(
    "complete_carplay:displayText",
    function(msg, type)
        DisplayTextMessage(msg, type)
    end
)

RegisterCommand(Config.CarplayInstallCommand, function()
...
function InstallCarplay()
    ...
    if IsPedInAnyVehicle(player_ped, false) then
        ...
        local carplay_quantity = QBCore.Functions.HasItem('carplay')
        if carplay_quantity == true then
        ...
        QBCore.Functions.TriggerCallback(
            ...
            function(result)
                if result then
                    ...
                    DisplayTextMessage("You're not allowed to use this command", "error")
                end
            end
        )
    else
...
RegisterNUICallback(
    ...
    function(data)
        ...
        for i,b_song in ipairs(Config.BlackListSongs) do
            if url == b_song then
                DisplayTextMessage("You can't play that song", "error")
                return
            end
        end
        local nameid = nil
        ...
        local complete_url = "https://www.youtube.com/watch?v=" .. url
        SetUrl(complete_url, nameid)
    end
)
...
RegisterNUICallback(
    ...
    function(data)
        if Config.PhoneType == 'gcphone' then
            exports.gcphone:sendMessage(data["receiver"], data["message"])
        ...
        elseif Config.PhoneType == 'dphone' then
            TriggerServerEvent('complete_carplay:sendMessageDphone', data["receiver"], data["message"])
        ...
        elseif Config.PhoneType == 'chezza' then
            TriggerServerEvent("complete_carplay:sendMessageChezza", data["receiver"], data["message"], "[]")
    ...
    if Config.EnableNavigator then
        while true do
            if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                ...
                current_time = GetClockHours() .. ":" .. GetClockMinutes()
                ...
                SendNUIMessage(
                    {
                        type = "refresh_main",
                        current_fuel = exports["cdn-fuel"]:GetFuel(veh),
    ...
    if Config.DisplayCurrentStreet then
        ...
        while true do
            ...
            if veh ~= 0 then
                ...
                local v_plate = GetVehicleNumberPlateText(veh)
                if trunk_camera then
                    RemoveTrunkCamera()
                end
            end
...
function SetSetting(setting, bool)
    if setting == "headlight" then
        if bool then
            SetVehicleLightsMode(vehicle_opened, 2)
        else
            SetVehicleLightsMode(vehicle_opened, 1)
        end
...
-- https://www.youtube.com/watch?v=lngUYDhzsWM
...
AddEventHandler("complete_carplay:SetQueue", function(nome, queue, url)
	for i = 1, #music_zones do
		...
		if v.name == nome then
            ...
            local comp_url = "https://www.youtube.com/watch?v=" .. url
            SetUrl(comp_url, nome)
		end
	end
end)
